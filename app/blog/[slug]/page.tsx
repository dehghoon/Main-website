...
export default function ArticlePage(){
 ...
 return <main className="publicArticle">
  ...
  <article className="articleShell">
   ...
   <section className="sharePanel"><div className="sharePanelTitle"><strong>Share this article</strong><span>Uses the approved Social Media Marketing copy</span></div><div className="socialSharePreview">{article.cover_image_url&&<img src={article.cover_image_url} alt={article.cover_image_alt||article.title}/>}<div><strong>{article.title}</strong><p>{socialCopy}</p></div></div><div className="shareButtons"><button type="button" className="linkedinShare" onClick={shareLinkedIn}>LinkedIn</button><a href={`https://twitter.com/intent/tweet?text=${socialText}`} target="_blank" rel="noreferrer">X</a><a href={`https://t.me/share/url?url=${encoded}&text=${encodeURIComponent(socialCopy)}`} target="_blank" rel="noreferrer">Telegram</a><a href={`https://www.facebook.com/sharer/sharer.php?u=${encoded}`} target="_blank" rel="noreferrer">Facebook</a><a href={`https://www.reddit.com/submit?url=${encoded}&title=${titleText}`} target="_blank" rel="noreferrer">Reddit</a><a href={`mailto:?subject=${titleText}&body=${socialText}`}>Email</a><button type="button" onClick={shareInstagram}>Instagram Caption</button><button type="button" onClick={copySocial}>Copy Social Post</button><button type="button" onClick={copyLink}>Copy Link</button></div>{copied&&<div className="shareFeedback">{copied}</div>}</section>
  </article>
 </main>;
}
