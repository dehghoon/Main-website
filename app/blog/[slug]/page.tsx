...
type Article={title:string;excerpt:string|null;body:string;published_at:string|null;created_at:string;cover_image_url:string|null;video_url:string|null;cover_image_alt:string|null;cover_image_title:string|null;cover_image_caption:string|null;cover_image_meta:string|null;cover_image_position:string|null;cover_image_size:string|null;video_title:string|null;video_caption:string|null;video_thumbnail_url:string|null;video_meta:string|null;tags:string[];calculator_name:string|null;calculator_url:string|null;seo_title:string|null;seo_description:string|null};
function authorFromBody(html:string){const m=html.match(/<span data-blog-author="([^"]*)"[^>]*><\/span>/i);if(!m?.[1])return "";const t=document.createElement("textarea");t.innerHTML=m[1];return t.value;}
...
export default function ArticlePage(){
 ...
 useEffect(()=>{const s=getSupabase();if(!s)return setStatus("Article service is unavailable.");void s.from("blog_posts").select("title,excerpt,body,published_at,created_at,cover_image_url,video_url,cover_image_alt,cover_image_title,cover_image_caption,cover_image_meta,cover_image_position,cover_image_size,video_title,video_caption,video_thumbnail_url,video_meta,tags,calculator_name,calculator_url,seo_title,seo_description").eq("slug",params.slug).eq("status","published").single().then(({data,error})=>{if(error||!data)setStatus("Article not found.");else{setArticle(data as Article);setStatus("");document.title=(data.seo_title||data.title)+" | LinkoTech";const meta=document.querySelector('meta[name="description"]');if(meta)meta.setAttribute("content",data.seo_description||data.excerpt||"");}});},[params.slug]);
 if(!article)return <main className="publicArticle"><p>{status}</p></main>;
 const url=typeof window!=="undefined"?window.location.href:"https://linkoteq.com/blog"; const encoded=encodeURIComponent(url); const text=encodeURIComponent(article.title); const author=authorFromBody(article.body);
 ...
 return <main className="publicArticle">
  ...
  <article className="articleShell">
   ...
   <div className="articleMeta">{author&&<>{author} · </>}{new Date(article.published_at||article.created_at).toLocaleDateString()}</div><h1>{article.title}</h1>{article.excerpt&&<p className="articleLead">{article.excerpt}</p>}
   ...
   <section className="sharePanel"><div className="sharePanelTitle"><strong>Share this article</strong><span>Published content only</span></div><div className="shareButtons"><a className="linkedinShare" href={`https://www.linkedin.com/sharing/share-offsite/?url=${encoded}`} target="_blank" rel="noreferrer">LinkedIn</a><a href={`https://twitter.com/intent/tweet?url=${encoded}&text=${text}`} target="_blank" rel="noreferrer">X</a><a href={`https://t.me/share/url?url=${encoded}&text=${text}`} target="_blank" rel="noreferrer">Telegram</a><a href={`https://www.facebook.com/sharer/sharer.php?u=${encoded}`} target="_blank" rel="noreferrer">Facebook</a><a href={`https://www.reddit.com/submit?url=${encoded}&title=${text}`} target="_blank" rel="noreferrer">Reddit</a><a href={`mailto:?subject=${text}&body=${encoded}`}>Email</a><button type="button" onClick={copyInstagram}>Instagram Caption</button><button type="button" onClick={copyLink}>Copy Link</button></div>{copied&&<div className="shareFeedback">{copied}</div>}</section>
  </article>
 </main>;
}
